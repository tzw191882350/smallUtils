...
public class AbstractQueryAppServiceFileUtils {
     ...
     * 修改文件内容：字符串逐行替换
     */
    private static boolean modifyFileContent(final File file, final String sufstr, final String newStr) {
        List<String> list = null;
        try {
            final File newFile = new File(CreateFileConst.CREATEFILEPATH + "appservice\\abs\\Abstract"+ newStr + sufstr + ".java");
            if (newFile.exists()) {
                System.out.println(newFile + "文件已存在不作处理。");
                return true;
            }
            final String packageStr = CreateFileConst.CREATEFILEPATH.replace("\\", ".") + "appservice.abs";
    ...
    /**
     * 修改文件内容
     * @Param [filePath, oldstr, newStr]
     * @return boolean
     */
    private static boolean modifyFileContent(final String filePath, final String oldstr, final String newStr) {
